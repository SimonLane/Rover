...
class Rover(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.xbox_thread = Thread(
            target=self._xbox_meh,
...
# Connect to TEENSY BOARDS        
        ...
        # No need to locks in init function because nothing will use the device until it is discovered.
...
# task management       
        ...
        # Now start the xbox controller polling thread
    ...
    def _xbox_meh(self):
        ...
        self._playerno = pygame.joystick.get_count()
        print ("No. of controllers =", self._playerno)
        if self._playerno == 0:
            raise RuntimeError("Not enought XBOX controllers")

        self.player_list = []

        # Initialise controllers and get feedback
        for i in range (self._playerno):
            self.player_list.append(jy(i))
            # Make sure each controller gets initialised
            self.player_list[-1].init()
            ...
            self.player_list[-1].rumble(0.5,0.9,500)
            time.sleep(0.01)
        print("*"*80)
        while True:
            pass
    ...
    def serial_arm_command_with_response(self, command_str):
        with self.armLock:
            self.arm.write(str.encode(command_str))
            return self.arm.readline().strip()

    def update_power_arm(self):
        reply = self.serial_arm_command_with_response("/report.power;")
